...
class Teachers extends Controller
{
    ...
    {
        $data = [
            'payslip' => $this->commonModel->getSinglePayslip($_SESSION['user_id']),
            'user' => $this->commonModel->getUser($_SESSION['user_id']),
            'allowances' => $this->commonModel->getAllowances($_SESSION['user_id']),
            'deductions' => $this->commonModel->getDeductions($_SESSION['user_id']),
        ];
        $user_role = $data['user']->designation;
        if ($user_role == 'Teacher') {
            $teacher = $this->commonModel->getTeacherById($_SESSION['user_id']);
            $data['school'] = $teacher->school;
        } else if ($user_role == 'Principal') {
            $principal = $this->commonModel->getPrincipalById($_SESSION['user_id']);
