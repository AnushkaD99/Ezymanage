...
class Common
{
  ...
  public function addProject($data)
  {
    ...
    $this->db->query('INSERT INTO project_tbl
                        (name, description, school_name, drive_link, estimate_date) 
                        VALUES (:name, :description, :school_name, :drive_link, :estimate_date)');
    // Bind values
   ...
   {
     //$this->db->query('SELECT * FROM project_tbl WHERE project_id = :project_id ORDER BY date_added' );
     $this->db->query('SELECT * FROM project_tbl' );
     $results = $this->db->resultSet();
     return $results;
   }
  ...
  //Project
  public function getProjectDeatail()
  {
    $this->db->query('SELECT * FROM project_tbl ORDER BY 
                    CASE project_tbl.status 
                    WHEN 0 THEN 0 
                    WHEN 1 THEN 1 
                    ELSE 2 
                    END ASC ');
  ...
  {
    $this->db->query('SELECT * FROM project_tbl WHERE project_id = :id');
    $this->db->bind(':id', $id);
  ...
  {
    $this->db->query('SELECT * FROM project_tbl WHERE school_name LIKE :school_name OR name LIKE :project_name');

    //bind values
    $this->db->bind(':school_name', $data['search']);
    $this->db->bind(':project_name', $data['search']);
  ...
  {
    $this->db->query('SELECT * FROM appointment_tbl WHERE date LIKE :date ');
